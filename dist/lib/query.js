...
var query = function query(url, options) {
  ...
  return fetcher(resource, { agent: https.globalAgent }).then(checkStatus).then(toJson).then(toHtml);
};
...
var get = function get(resource) {
  ...
  return fetcher(resource, { agent: https.globalAgent }).then(checkStatus).then(toText).then(function (html) {
    return { html: html };
  });
};
